...
- (id)initWithStyle:(UITableViewStyle)style
{
    ...
    if (self) {
        NSArray *titles = [NSArray arrayWithObjects:@"Access",@"Excel",@"Outlook", @"Powerpoint",@"Word", nil];
        NSArray *images = [NSArray arrayWithObjects:[UIImage imageNamed:@"Access.png"],[UIImage imageNamed:@"Excel.png"], [UIImage imageNamed:@"Outlook.png"],[UIImage imageNamed:@"Powerpoint.png"],[UIImage imageNamed:@"Word.png"], nil];
        
        pictureDict = [[NSDictionary alloc] initWithObjects:images forKeys:titles];
        
        [self setTitle:@"Microsoft"];
        
        if ([[UIScreen mainScreen] respondsToSelector:@selector(scale)] && [[UIScreen mainScreen] scale] == 2){
            self.tabBarItem.image = [UIImage imageNamed:@"monitor retina.png"];
        }
        else
        {
            self.tabBarItem.image = [UIImage imageNamed:@"monitor.png"];
        }
    }
    return self;
}
